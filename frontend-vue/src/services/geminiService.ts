...
const getConfig = (): AISettings => {
  ...
  // 注意：import.meta.env.VITE_API_KEY 是构建时注入或运行时提供的环境变量
  const effectiveApiKey = settings.apiKey || import.meta.env.VITE_API_KEY || '';
 ...
 */
export function parseAsArray<T>(text: string): T[] {
  try {
    // 1. 移除 Markdown 代码块标记
